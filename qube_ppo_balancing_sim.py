...
if __name__=="__main__":
    env = QubeBalanceEnv(use_simulator=True)    
    env.seed(500)
    ...
    for iteration in range(1,15000):
        ...
        if iteration % 50 == 0:
            plt.savefig('C:/Users/dohyeok/Desktop/19_urp/code_test/qube_simul/result_img'+str(iteration)+'.png')
            
            with open('C:/Users/dohyeok/Desktop/19_urp/code_test/qube_simul/result_data/bal_sim_ppo_result_reward_'+str(iteration)+'.pickle','wb') as savedata:
                pickle.dump([iteration,reward_list],savedata)
            torch.save(actor.state_dict(),'./ppo_bal_sim'+str(iteration)+'.pth')
    env.close()    
